		...
		try {
			...
			if (clientMessage.getMessageType() == MessageType.NEW_NODE) {
				...
				if (AppConfig.chordState.isCollision(newNodeInfo.getChordId())) {
					...
					// also the new node that was trying to join should give Suzuki token to this node (or next in queue)
					// and notify bootstrap server that it failed to join (so that bootstrap can allow other nodes to join)
					return;
				}
